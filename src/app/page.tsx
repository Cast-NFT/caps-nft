...
export default function Home() {
  ...
  const price = 0.0001;
  const totalPrice = useMemo(() => (mintCount * price).toFixed(4), [mintCount]);
        ...
        <header className="sticky top-0 z-30 border-b border-white/8 bg-[#07060a]/85 backdrop-blur-xl">
          <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
            ...
            <div className="hidden items-center gap-3 lg:flex motion-rise motion-delay-2">
              <span className="rounded-full border border-gold/25 bg-gold/10 px-3 py-1 font-mono text-[11px] uppercase tracking-[0.18em] text-gold">
                Mint Coming Soon
              </span>
        ...
        <section className="mx-auto grid max-w-7xl grid-cols-1 gap-12 px-5 pb-16 pt-12 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:pb-24 lg:pt-20">
          <div className="flex flex-col justify-center motion-rise">
            ...
            <div className="hero-copy">
              ...
              <p className="mt-8 max-w-2xl text-[15px] leading-8 text-text-soft sm:text-[17px]">
                EchoCapsul is a time capsule NFT collection on Base. Every holder starts with a sealed
                Mystery Box. Once mint closes, each box opens into a Capsule with its own rarity,
                shell, traits, and forge potential — while the Inner Archive stays locked.
              </p>
            </div>

            <div className="mt-9 flex flex-wrap gap-3">
              <a href="#mint" className="rounded-2xl bg-white px-6 py-4 text-base font-semibold text-black shadow-[0_0_0_1px_rgba(255,255,255,0.14),0_18px_40px_rgba(255,255,255,0.08)] transition hover:opacity-92">
                Open Mint Panel
              </a>
              <a href="#reveal" className="rounded-xl border border-white/10 bg-white/5 px-5 py-3 text-sm font-medium text-white transition hover:bg-white/8">
                View Reveal Spectrum
              </a>
              <Link href="/gallery" className="rounded-xl border border-echo/20 bg-echo/10 px-5 py-3 text-sm font-medium text-white transition hover:bg-echo/15">
                Open Gallery
              </Link>
            </div>
            ...
            <div className="mt-10 grid max-w-xl grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <MetricCard label="Supply" value="10,000" note="target collection size" />
            </div>
          </div>
          ...
          <div className="glass-panel rounded-[30px] p-4 sm:p-5 motion-rise motion-delay-1">
            <div className="mb-4 flex items-center justify-between border-b border-white/8 pb-4">
              ...
              <div className="rounded-full border border-echo/20 bg-echo/10 px-3 py-1 font-mono text-[11px] uppercase tracking-[0.18em] text-echo">
                {selectedCapsule.tier}
              </div>
            </div>
            ...
            <div className={`relative overflow-hidden rounded-[26px] border border-white/8 bg-gradient-to-b ${selectedCapsule.accent} from-0% via-transparent to-transparent p-4 sm:p-6`}>
              ...
              <div className="relative flex min-h-[320px] items-center justify-center sm:min-h-[460px]">
                <Image
                  ...
                  height={430}
                  priority
                  className="h-auto max-h-[300px] w-full max-w-[300px] object-contain sm:max-h-[430px] sm:max-w-[430px]"
                  unoptimized
                />
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-6">
              {capsules.map((capsule) => (
                <button
                  ...
                  className={`rounded-2xl border p-2 transition ${selectedCapsule.id === capsule.id ? "border-echo bg-echo/10" : "border-white/8 bg-white/4 hover:bg-white/7"}`}
                >
                  <Image
                    src={capsule.image}
                    alt={capsule.name}
            ...
            <div className="grid grid-cols-1 gap-8 xl:grid-cols-[0.88fr_1.12fr]">
              <div>
                ...
                <h2 className="mt-3 text-3xl font-semibold tracking-[-0.04em] text-white">
                  One clean path from sealed mint to live contract.
                </h2>
                <p className="mt-3 max-w-xl text-sm leading-7 text-text-soft">
                  The mint interface is visible now so the collection flow is easy to understand.
                  Transaction wiring comes next. For now, price, supply, phase, and wallet surface are
                  presented in one place instead of split across disconnected panels.
                </p>

                <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
                  <Stat label="Minted" value={`${minted}`} detail={`/ ${maxSupply}`} />
                  <Stat label="Price" value={`${price}`} detail="ETH" />
                  <Stat label="Phase" value="I" detail="Sealed Mint" />
                </div>
              </div>
              ...
              <div>
                <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="ml-2 font-mono text-[11px] uppercase tracking-[0.22em] text-text-dim">
                      mint.echocapsul
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 font-mono text-[11px] uppercase tracking-[0.18em] text-text-soft">
                      Preview Only
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-5 flex flex-col gap-4 rounded-2xl border border-white/8 bg-white/3 p-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={() => setMintCount((prev) => Math.max(1, prev - 1))} className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-lg text-white hover:bg-white/10">
                      −
                    </button>
                    <div className="min-w-14 text-center text-2xl font-semibold tracking-[-0.04em] text-white">{mintCount}</div>
                    <button onClick={() => setMintCount((prev) => Math.min(10, prev + 1))} className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-lg text-white hover:bg-white/10">
                      +
                    </button>
                  </div>
                </div>
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-3">
                  ...
                  <div className="rounded-2xl border border-white/8 bg-white/3 p-4">
                    <div className="font-mono text-[11px] uppercase tracking-[0.22em] text-text-dim">chain</div>
                    <div className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-white">Base</div>
                  </div>
                </div>

                <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-center">
                  <button disabled className="w-full cursor-not-allowed rounded-2xl border border-white/10 bg-white/10 px-5 py-4 text-sm font-semibold text-white/55">
                    MINT NOT LIVE YET
                  </button>
                ...
                <p className="mt-3 text-center font-mono text-[11px] uppercase tracking-[0.22em] text-text-dim sm:text-left">
                  wallet connection is shown here for interface preview only
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="mx-auto max-w-7xl px-5 py-20 lg:px-8" id="reveal">
          <div className="mb-8 max-w-2xl motion-rise">
            ...
            <h2 className="mt-3 text-4xl font-semibold tracking-[-0.05em] text-white">
              The shell opens. The archive stays sealed.
            </h2>
            <p className="mt-4 text-sm leading-7 text-text-soft sm:text-base">
              Reveal is where the collection splits into real market identities. Tier, shell, traits,
              and forge potential become visible. The Inner Archive does not.
            </p>
          </div>
        ...
        <section className="mx-auto max-w-7xl px-5 pb-20 lg:px-8" id="furnace">
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-[0.84fr_1.16fr]">
            <div className="glass-panel rounded-[30px] p-6 sm:p-8 motion-rise">
              ...
              <h2 className="mt-3 text-4xl font-semibold tracking-[-0.05em] text-white">
                Supply leaves. Story stays.
              </h2>
              <p className="mt-4 text-sm leading-7 text-text-soft sm:text-base">
                The Furnace turns lower tiers into higher forms while preserving origin history.
                That means Common remains useful, forged assets feel earned, and Mythic stays rare
                enough to matter.
              </p>
            ...
            <div className="glass-panel rounded-[30px] p-6 sm:p-8 motion-rise motion-delay-1">
              <div className="mb-5 flex items-center justify-between">
                <div>
                  ...
                  <div className="font-mono text-[11px] uppercase tracking-[0.22em] text-text-dim">
                    tier flow // burn // upgrade
                  </div>
                </div>
                <div className="rounded-full border border-gold/25 bg-gold/10 px-3 py-1 font-mono text-[11px] uppercase tracking-[0.18em] text-gold">
                  active in phase II
                </div>
              </div>
          ...
          <div className="glass-panel rounded-[32px] p-6 sm:p-8 lg:p-10 motion-rise">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h2 className="mt-3 text-4xl font-semibold tracking-[-0.05em] text-white">
                  A roadmap with structure, not empty promises.
                </h2>
              </div>
              <div className="max-w-md text-sm leading-7 text-text-soft">
                The focus stays on mint, reveal, market formation, and forge utility. The Inner Archive
                exists as a deeper layer, not as vague future utility.
              </div>
            </div>
            ...
            <div className="mt-8 grid gap-4 lg:grid-cols-5">
              {roadmap.map((item) => {
                ...
                return (
                  <div
                    key={item.title}
                    className={`rounded-[24px] p-5 soft-card ${
                      active ? "roadmap-active" : sealed ? "roadmap-sealed" : "roadmap-locked"
                    }`}
                  >
                    <div className="inline-flex rounded-full border border-white/10 bg-white/5 px-3 py-1 font-mono text-[11px] uppercase tracking-[0.2em] text-text-soft">
                      {active ? "● ACTIVE" : sealed ? "◌ SEALED" : "◦ LOCKED"}
                    </div>
        ...
        <footer className="border-t border-white/8 px-5 py-8 lg:px-8">
          <div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-[1fr_auto_auto] md:items-start">
            <div>
              ...
              <div className="mt-1 font-mono text-[11px] uppercase tracking-[0.22em] text-text-dim">
                base network // phase i build
              </div>
              <p className="mt-4 max-w-md text-sm leading-7 text-text-soft">
                A sealed time capsule NFT system built around reveal, market structure, forging, and a
                deeper archive layer that stays locked until it matters.
              </p>
            </div>
